...
const config = convict({
	...
	// How toolbox can be reached (Editor & REST-API)
	path: {
		format: String,
		default: '/',
		arg: 'path',
		env: 'toolbox_PATH',
		doc: 'Path toolbox is deployed to'
	},
	host: {
		format: String,
		default: 'localhost',
		arg: 'host',
		env: 'toolbox_HOST',
		doc: 'Host name toolbox can be reached'
	},
	port: {
		format: Number,
		default: 5678,
		arg: 'port',
		env: 'toolbox_PORT',
		doc: 'HTTP port toolbox can be reached'
	},
	listen_address: {
		format: String,
		default: '0.0.0.0',
		env: 'toolbox_LISTEN_ADDRESS',
		doc: 'IP address toolbox should listen on'
	},
	protocol: {
		format: ['http', 'https'],
		default: 'http',
		env: 'toolbox_PROTOCOL',
		doc: 'HTTP Protocol via which toolbox can be reached'
	},
	ssl_key: {
		format: String,
		default: '',
	...
	endpoints: {
		rest: {
			format: String,
			default: 'rest',
			env: 'toolbox_ENDPOINT_REST',
			doc: 'Path for rest endpoint'
		},
		webhook: {
			format: String,
			default: 'webhook',
			env: 'toolbox_ENDPOINT_WEBHOOK',
			doc: 'Path for webhook endpoint'
		},
		webhookTest: {
			format: String,
			default: 'webhook-test',
			env: 'toolbox_ENDPOINT_WEBHOOK_TEST',
	...
	nodes: {
		...
		errorTriggerType: {
			...
			format: String,
			default: 'toolbox-nodes-base.errorTrigger',
...
if (process.env.toolbox_CONFIG_FILES !== undefined) {
	const configFiles = process.env.toolbox_CONFIG_FILES.split(',');
