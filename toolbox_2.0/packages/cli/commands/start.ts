...
export class Start extends Command {
	static description = 'Starts toolbox. Makes Web-UI available and starts active workflows';

	static examples = [
		`$ toolbox start`,
		`$ toolbox start --tunnel`,
		`$ toolbox start -o`,
		`$ toolbox start --tunnel -o`,
	];
	...
	static flags = {
		...
		tunnel: flags.boolean({
			description: 'runs the webhooks via a hooks.toolbox.cloud tunnel server. Use only for testing and development!',
		}),
	};
	...
	/**
	 * Stoppes the toolbox in a graceful way.
	...
	async run() {
		// Make sure that toolbox shuts down gracefully if possible
			...
			try {
				...
				if (flags.tunnel === true) {
					...
					const tunnelSettings: localtunnel.TunnelConfig = {
						host: 'https://hooks.toolbox.cloud',
					...
					this.log('IMPORTANT! Do not share with anybody as it would give people access to your toolbox instance!');
				}
				...
				// Allow to open toolbox editor by pressing "o"
