	...
	.extend({
		...
		methods: {
				...
				if (this.isReadOnly === false) {
					connectionOverlays.push.apply(connectionOverlays, [
						...
						[
							...
							{
								id: 'drop-add-node',
								label: 'Drop connection<br />to create worker',
			...
			removeNode (nodeName: string) {
				...
				const requiredNodeTypes = [ 'toolbox-nodes-base.start' ];
			...
			async addNodes (nodes: INodeUi[], connections?: IConnections) {
				...
				nodes.forEach((node) => {
					...
					if (nodeType !== null) {
						...
						if (node.type === 'toolbox-nodes-base.webhook' && node.parameters.path === '') {
			...
			async loadSettings (): Promise<void> {
				const settings = await this.restApi().getSettings() as ItoolboxUISettings;
