...
export async function getUserSettings(settingsPath?: string, ignoreCache?: boolean): Promise<IUserSettings | undefined> {
	...
	try {
		...
		throw new Error(`Error parsing toolbox-config file "${settingsPath}". It does not seem to be valid JSON.`);
	}
...
export function getUserSettingsPath(): string {
	const toolboxFolder = getUsertoolboxFolderPath();

	return path.join(toolboxFolder, USER_SETTINGS_FILE_NAME);
}



/**
 * Retruns the path to the toolbox folder in which all toolbox
 * related data gets saved
 *
 * @export
 * @returns {string}
 */
export function getUsertoolboxFolderPath(): string {
	let userFolder;
...
/**
 * Returns the path to the toolbox user folder with the custom
 * extensions like nodes and credentials
 *
 * @export
 * @returns {string}
 */
export function getUsertoolboxFolderCustomExtensionPath(): string {
	return path.join(getUsertoolboxFolderPath(), EXTENSIONS_SUBDIRECTORY);
}
